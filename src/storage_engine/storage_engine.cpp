...
void storage_engine::createTable(create_tbl_expr *expr) {
  ...
  for (const auto &column : expr->tblColumns) {
    if (!isTypeValid(column.type))
      throw std::invalid_argument("Invalid type detected! Wtf is: " +
                                  std::string(column.type));
    new_table.columns.emplace_back(std::string(column.name),
                                   fromString(column.type));
...
void storage_engine::queryDataFromTable(select_expr *expr) {
  ...
  if (columnsToFetch.empty()) {
    if (!expr->tblName.empty()) {
      ...
      if (it == tables.end())
        throw std::invalid_argument("Table: " + std::string(expr->tblName) +
                                    " doesn't exist!");
    }
    auto value = expr->execute();
    std::visit(
        overload{[](const std::vector<exprs::values> &v) {
                   for (const auto &vl : v) {
                     std::visit(
                         overload{
  ...
  for (const auto &column : columnsToFetch) {
    ...
    if (!found)
      throw std::invalid_argument("Couldn't find the column " +
                                  std::string(column) + "in table named" +
                                  tblName);
  }
...
void storage_engine::insertIntoTable(insert_expr *expr) {
  ...
  for (const auto exprColumn : expr->columns) {
    ...
    if (!found)
      throw std::invalid_argument("Couldn't find the column " +
                                  std::string(exprColumn) + "in table named" +
                                  tblName);
  }
