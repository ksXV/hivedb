...
namespace hivedb {
    ...
    disk_manager::disk_manager(const std::filesystem::path& db_path): m_db_file(), m_db_file_path(db_path) {
        m_db_file.open(db_path, std::ios::binary | std::ios::in | std::ios::out);
        if (!m_db_file.is_open()) {
            m_db_file.open(db_path, std::ios::trunc | std::ios::binary | std::ios::in | std::ios::out);
            if (!m_db_file.is_open()) throw std::runtime_error("failed to open the file!");
        }

        //the first page (or the + 1) is reserved for the system
    ...
    void disk_manager::write_page(page_id_t id, const char* buffer) {
        if (id < 0) throw std::runtime_error("Invalid id detected!: " + std::to_string(id));
        offset_t offset = m_pages.find(id) != m_pages.end() ? allocate_new_page() : m_pages[id];
    ...
    void disk_manager::read_page(page_id_t id, char* buffer) {
        ...
        if (offset > file_size) {
            throw std::runtime_error("Offset was bigger (somehow) than file size! offset: " + std::to_string(offset) + " file_size: " + std::to_string(file_size));
        }
        ...
        if (PAGE_SIZE > read_count) {
           fmt::println("warning: couldn't read full page! read: {}", read_count);
    ...
    void disk_manager::delete_page(page_id_t id) {
        if (id < 0) throw std::runtime_error("Invalid id detected!: " + std::to_string(id));
        if (m_pages.find(id) == m_pages.end()) throw std::runtime_error("Invalid id detected!: " + std::to_string(id));
    ...
    offset_t disk_manager::allocate_new_page() {
        ...
        if (m_pages.size() + 1 >= m_current_pages) {
            m_current_pages *= 2;
            std::filesystem::resize_file(m_db_file_path, (m_current_pages+1) * PAGE_SIZE);
        }
    ...
    std::size_t disk_manager::get_file_size() {
        ...
        if (status == -1) {
            throw std::runtime_error("Failed to fetch the file size! ERRNO: " + std::to_string(errno));
        }
